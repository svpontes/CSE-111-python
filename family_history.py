# Cada valor no dicionario de pessoas é uma lista. These
# estes são os indexes dos elementos nas listas.
from template import print_cylinder_volume
...
# Cada value no dicionario de casamentos é uma lista.
# estes são os indexes dos elementos nesta lista.
HUSBAND_KEY_INDEX = 0
WIFE_KEY_INDEX = 1
...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    for person_key, person_list in people_dict():
...
def print_marriages(marriages_dict, people_dict):
    ...
    for persons_key, person_list in people_dict.items():
        name = person_list[NAME_INDEX]
