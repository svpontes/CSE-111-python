...
def main():
    gender = get_gender("Enter your gender (M or F): ")
...
def get_gender(prompt, female="F", male="M"):
    ...
    while True:
        try:
            ...
            if gender.lower() != female or male: 
                print(f"You entered {gender}. You must digit: M (male) or F (female)")
                print("Please enter F for Female or M for male.")
            else:
                break
            ...
            print(f"{gender} must be F (female) or M (male).")
            print("Please enter F for Female or M for Male.")
